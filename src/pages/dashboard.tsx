...
import { getUserFollowers } from 'apiSdk/user-followers';
...
function DashboardPage() {
  ...
  const userFollowersStat = useSWR<PaginatedInterface<UserFollowersInterface>>(
    () => `/user-followers`,
    () => getUserFollowers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: userFollowersStat?.data?.totalCount,
      isLoading: userFollowersStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
